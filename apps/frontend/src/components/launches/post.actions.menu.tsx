...
export const PostActionsMenu: FC<{
  ...
  return (
    <div
      ...
      onClick={toggle}
      onKeyDown={keyToggle}
      // No z-index here. Being positioned is already enough to put the strip
      // over the tag name behind it, and any value turns this into a stacking
      // context that the panel's own `z-[100]` then resolves inside — which put
      // the whole list back under the provider badge the card body hangs at
      // `z-10` (calendar.tsx:1218), so the icon painted through the open menu.
      className="absolute inset-0 flex items-center justify-end pe-[3px] rounded-tr-[10px] rounded-tl-[10px] cursor-pointer focus-visible:ring-2 focus-visible:ring-brand"
    >
      <svg
        ...
        >
          {actions.map((action) => (
            <div
              ...
              role="menuitem"
              tabIndex={0}
              className="flex gap-[12px] items-center py-[8px] px-[10px] rounded-[8px] cursor-pointer hover:bg-boxHover transition-colors"
              onClick={() => {
                setShow(false);
                action.onClick();
              }}
