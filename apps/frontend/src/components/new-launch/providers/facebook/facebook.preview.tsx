    ...
    const finalValue =
      newContent
        ...
        .replace(/\[\[\[([.\s\S]*?)]]]/, (match, match1) => {
          return `<span class="font-bold font-[arial]" style="color: var(--brand-text)">${match1}</span>`;
        }) +
      `<mark class="bg-error" data-tooltip-id="tooltip" data-tooltip-content="This text will be cropped">` +
    ...
    <div className="py-[15px] flex flex-col px-[15px] w-full gap-[20px] bg-bgFacebook rounded-[12px]">
      ...
      <div className="flex text-textLinkedin text-[12px] font-[400] items-center">
        <div className="flex flex-1 gap-[10px] items-center">
          <Icons />
          <div className="">{t('preview_you_and_others', 'You & 12 other')}</div>
        </div>
        <div className="gap-[9px] items-center flex">
          <div>{t('preview_facebook_comments', '20 Comments')}</div>
        </div>
      </div>
