...
const DeleteAccountComponent: FC<{ isLink?: boolean }> = ({ isLink }) => {
  ...
  const loadingOverlay = loading && (
    <div className="text-textColor fixed start-0 top-0 bg-[color-mix(in_srgb,var(--color-primary)_80%,transparent)] z-[500] w-full h-full animate-fade flex flex-col items-center justify-center gap-[24px]">
  ...
  if (isLink) {
    return (
      <>
        {loadingOverlay}
        <div
          className="cursor-pointer flex items-center gap-[8px] text-error hover:underline text-[14px]"
