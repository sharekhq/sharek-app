...
import { VignettesComponent } from '@gitroom/frontend/components/auth/vignettes.component';
...
export default async function AuthLayout({
  ...
  const t = await getT();
  // Resolved here rather than in the marquee so the browser fetches one
  // language's screenshots instead of both, and so they always match the
  // language getT() just rendered the headline in.
    ...
    <div className="bg-[#0E0E0E] flex flex-1 p-[12px] gap-[12px] min-h-screen w-screen text-white">
      ...
      <div className="text-[36px] flex-1 pt-[88px] hidden lg:flex flex-col items-center">
        <div className="text-center">
          {t('auth_headline_lead', 'Create with AI.')}
          <br />
          <span className="text-brand">
            {t('auth_headline_highlight', 'Share everywhere.')}
          </span>
        </div>
        <VignettesComponent language={language} />
      </div>
    </div>
  );
}
